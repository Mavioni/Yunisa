// layers/maritime.js â€” Real-time AIS vessel tracking
//
...
WV.layers.maritime = (function () {
  ...
  var socket          = null;
  var vessels         = {};   // MMSI â†’ {lat, lon, name, sog, heading, cog, status}
  var posHistory      = {};   // MMSI â†’ [{lat, lon}, ...]
  ...
  var MAX_HIST        = 20;
  var FOLLOW_ALT      = 150000;  // 150 km â€” lower than aircraft since ships are at sea level

  var WS_URL = 'wss://stream.aisstream.io/v0/stream';

  // REST fallback â€” Finland/Baltic open government AIS data, CORS-enabled, no key needed
  var REST_SOURCES = [
  ...
  // â”€â”€ SHIP ICON â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // Top-down hull: pointed bow (top / north at 0Â°), wider amidships, flat stern
  ...
  // â”€â”€ PATH DRAWING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  function select(mmsi) {
    ...
    WV.Controls.setStatus('TRACKING: ' + name + ' â€” CLICK GLOBE TO RELEASE');
  }
  ...
  // â”€â”€ VESSEL RENDERING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      ...
      billboardCollection.add({
        ...
        id: {
          ...
          _wvMeta: [
            ...
            { key: 'HEADING', val: v.heading !== null ? v.heading + 'Â°'   : '---' },
  ...
  // â”€â”€ WEBSOCKET (aisstream.io) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function connectWS(viewer) {
    ...
    socket.onclose = function (evt) {
      ...
      console.warn('maritime WS closed â€” code:', evt.code, 'reason:', evt.reason || '(none)');
      if (!enabled) return;

      // 1006 = connection failed â€” REST is already running so nothing more needed
  ...
  // â”€â”€ REST POLLING (digitraffic.fi) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function parseDigitraffic(data) {
    // GeoJSON FeatureCollection â€” coordinates: [lon, lat]
    ...
    // Plain array â€” handles both {x, y} and {longitude, latitude} field names
  ...
  function fetchREST(viewer) {
    if (restSourceIdx >= REST_SOURCES.length) {
      WV.Controls.setStatus('MARITIME: all sources failed â€” no vessel data available');
      return;
    }
    ...
    fetchWithTimeout(url, 15000)
      ...
      .then(function (data) {
        ...
        WV.Controls.setStatus('MARITIME (REST): ' + parsed.length + ' vessels â€” Baltic/Nordic');
      })
      .catch(function (err) {
  ...
  // â”€â”€ ENABLE / DISABLE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  function enable(viewer) {
    enabled       = true;
    wsRetries     = 0;
    restSourceIdx = 0;
    // Start REST immediately â€” vessels appear fast regardless of WebSocket status
    startREST(viewer);
    // WebSocket (aisstream.io) disabled â€” uncomment if key is working:
